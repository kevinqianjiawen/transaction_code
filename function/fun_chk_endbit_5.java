public boolean fun_chk_endbit_4(){
			int sum,i,j,k;
			 String fin_prodno;
			    String pl3_prod_id_2 = this.curTranSession.getVariableList().getValue("pl3_prod_id_2");
		    ...
		    for (i=0 ;i<= 5 ;i++)
		     { k=pl6_inst_no_4[i]-48;
		      sum=sum + ((sav_chk:j)*k);
		      j=j+1;
		     }
		    for ( i=0 ;i<= 1 ;i++)
		     { k=pl2_curr_id_5[i]-48;
		      sum=sum + ((sav_chk:j)*k);
		      j=j+1;
		     }
		    
		    if ( pl3_prod_id_2 == null )
		     { fin_prodno = p3_prod_id_2;
		     }
		    else
		     {
		     fin_prodno = pl3_prod_id_2;
		     }
